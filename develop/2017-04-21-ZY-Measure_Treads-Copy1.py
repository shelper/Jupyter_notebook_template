...
folder = r'C:\Users\MPNV38\ZDevelop\tiretread\data\test_data\from_Neeharika'
files = glob.glob(os.path.join(folder, '*.bmp'))

for i, img_file in enumerate(files[1:2]):
    ...
    treads_edge = treads_edge[(min_tread_width <= treads_width) * (treads_width <= max_tread_width)]
    ...
    picked_treads = treads[treads_score.argsort()[-treads_num:]]
